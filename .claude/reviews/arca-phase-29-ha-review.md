...
**Update 2026-06-11 (security).** A focused inter-node trust pass (§3.7), prompted by Pietro asking whether a sniffer could read `[cluster].secret` and impersonate a node, found that the secret never travels in cleartext (SigV4) — but a rogue peer receives all *new* data with no secret at all, because the fan-out push direction authenticates no peer; with a weak secret (allowed, and the shipped defaults are weak) full impersonation is cheap. It lowers the inter-node security score and adds a security-critical workstream to the action plan.

**Update 2026-06-11 (design).** A second pass dedicated to the plan as a design document (§6, findings D1–D12) refines the judgment: the decisions that were taken remain excellent, but the plan errs by omission. It does not design the failure paths of the guarantees it declares (the root of §2.1), it does not cover the operational lifecycle of the cluster (resize, secret rotation, restore from backup), it does not audit the pre-existing subsystems against N-node execution (the root of §3.3 and §3.6), and it never verifies a real partition. The dedicated assessment row is in the table.
...
| Inter-node security | 4/10 | Secret never on the wire (SigV4) — but the fan-out push direction authenticates no peer, so an mDNS rogue receives all new data with no secret (§3.7); plus brute-forceable fingerprint, weak secrets allowed, no anti-replay, unverified TLS |
...
### 3.7 A rogue peer receives all new data with no secret, and full impersonation is cheap

This finding consolidates the inter-node trust analysis (verified 2026-06-11 at Pietro's request: "can a sniffer read `[cluster].secret` and impersonate a rogue node receiving all data?"). The literal worry — the secret travelling in cleartext — does **not** happen: discovery announces only `cluster_id`/`node_id`, not the secret (`membership.rs:187-190`), and inter-node auth is SigV4, where the secret is the HMAC signing *key* and never goes on the wire (`client.rs:338-345`, verified in `cluster_auth.rs:155-166`). A passive sniffer does not directly read the secret. But the underlying concern is valid through three paths, the first of which needs no secret at all.

**(A) Fan-out to undiscriminated peers — a rogue receives all new writes without the secret.** Membership adds an mDNS peer purely on a `cluster_id` TXT match, with no proof of secret possession (`membership.rs:219-224`); liveness and identity come from the unauthenticated `GET /cluster/v1/health` (`router.rs:223`); and `live_peers()` — the fan-out target list — filters only on `alive`, never on `config_ok` (`cluster_meta.rs:73-80`, `cluster_blob.rs:65-70`). The fan-out then pushes every new blob's bytes and metadata row to those peers (`cluster_blob.rs:77,107`, `cluster_meta.rs:99,114,148`). The sender signs the request (proving *itself* to the receiver) but never authenticates the *receiver*; TD-015 means TLS does not authenticate it either. So a process on the cluster LAN advertising `_arca._tcp.local` with the right (non-secret, often guessable) `cluster_id`, answering `/health` with any `node_id`, is added as a live peer and **receives every newly written object and metadata row in real time, without ever needing the secret.** The receive direction is protected by `cluster_auth` (the rogue cannot *pull* history or *push* to legit nodes — those need the secret), but the *push* direction has no peer authentication. The only existing signal is a config-mismatch `tracing::warn!` (`membership.rs:118-125`), and the rogue evades even that by omitting the fingerprint, which makes `config_ok` default to `true` (`membership.rs:114-117`). Historical data is not exposed this way (anti-entropy is pull-based and requires signing).

**(B) The secret is brute-forceable offline from the public health endpoint.** `config_fingerprint` is a SHA-256 truncated to 8 bytes over `(cluster_id, mode, write_quorum, secret, master_key_id)` (`cluster.rs:56-77`), served unauthenticated on `/cluster/v1/health` and read by every peer (`membership.rs:255-258`). Every input other than the secret is low-entropy or guessable, so the fingerprint is an offline brute-force oracle for the secret. Combined with the absence of any minimum length or entropy requirement — config validation only rejects an *empty* secret (`config.rs:800-801`) — and the weak placeholder secrets shipped in the reference configs (`dev-cluster-secret-change-me`, `CHANGEME-CLUSTER-SECRET`), a weak secret is recoverable in seconds from the public endpoint alone, with no sniffing. Once the secret is known the rogue gains full impersonation: pull the whole history (manifest + every blob), push forged data, and pass `cluster_auth` everywhere.

**(C) Plain-HTTP transport by default, unverified TLS, no replay window.** The shipped cluster configs enable no TLS, so inter-node traffic is cleartext on the LAN; even with TLS, both the membership probe and the client disable certificate verification (`membership.rs:50`, `client.rs:74`; TD-015), so an active MITM is unimpeded. A sniffer therefore sees full signed requests and responses: it can brute-force a weak secret from a captured signature (as in B), and — because `cluster_auth` extracts `x-amz-date` but never checks it against the clock (`cluster_auth.rs:138-146`; cf. §3.1) — it can replay a captured signed `GET /cluster/v1/blob/{id}` or `/manifest` to pull that data with no secret at all.

**Severity.** High in the default posture (plain-HTTP LAN + mDNS + no secret-strength floor); effectively critical if the cluster's internal network is not a trusted, isolated segment. (A) is the most important: a real-time confidentiality breach of all new data that requires only LAN presence and a guessable `cluster_id`.

**Recommendation.** Two complementary fixes, both already partly in the hardening plan but under-weighted there:

- *Authenticate the peer before trusting it, in both directions.* The robust fix is **mutual TLS with a shared cluster CA** (closes the push direction of (A) and all of (C) at once, and is the proper resolution of TD-015): a node verifies the peer's CA-signed certificate before adding it to membership / fanning out. The secret-only fallback (for deployments that will not manage a CA) is an application-layer **challenge-response**: the peer must return a MAC/signature over a fresh nonce, proving secret possession, before it is counted live; `live_peers()` and the quorum then use only authenticated peers. This is *more* than "a signed `/ping`": the peer must prove possession to *us*, not merely accept our signed call (a rogue controls its own server and can return 200 unconditionally).
- *Remove the offline oracle and the weak secrets.* Move `config_fingerprint` off the public endpoint (§3.5), enforce a minimum length and high entropy on the secret and refuse the shipped placeholders (M5, strengthened), and add the anti-replay window (§3.1).

**(P1 — security-critical; treat the peer-authentication half with P0 urgency for any deployment whose cluster network is not fully trusted.)**
...
| **P1 (sec)** | §3.7(A) — Authenticate the peer before fan-out: mutual TLS w/ shared CA (preferred, also closes TD-015) or challenge-response proof-of-possession; `live_peers()` + quorum count only authenticated peers. **P0 urgency on untrusted networks** | 1–2 days |
| **P1 (sec)** | §3.7(B) — Move `config_fingerprint` off public health (§3.5); enforce secret min length + entropy and reject shipped placeholders (M5+) | Hours |
| **P1 (sec)** | §3.7(C) — Commit TD-015 (verified inter-node TLS w/ shared CA) + anti-replay window (§3.1) | 1 day |
...
A final, sharper point came from the inter-node trust pass (§3.7): the cluster authenticates the *sender* of every replication request but never the *receiver*, so the secret protects the inbound surface while the outbound fan-out will hand all new data to any peer membership discovered — and membership discovers an mDNS peer on a non-secret `cluster_id` match alone. The secret is well handled where it is used (never on the wire, HMAC-keyed signing, isolated credential); the gap is that whole half of the trust model — authenticating peers, not just requests — was never designed. Closing it (mutual TLS with a shared cluster CA being the cleanest move, as it also resolves TD-015 and the cleartext/MITM exposure) plus removing the brute-force oracle and the weak-secret allowance turns the inter-node surface from "high risk on an untrusted network" into something defensible. This is the security workstream now folded into the hardening plan (R3 + R4); on an untrusted cluster network its peer-authentication half deserves P0 urgency.

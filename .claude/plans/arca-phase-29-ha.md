...
- **Control plane — idempotent apply**: per-store `apply_remote_*` methods on `CredentialStore`/`UserStore`/`TeamStore`/`GrantStore`/`ServerConfigStore` + bucket (`buckets`/`bucket_config`/`bucket_tags`), applying verbatim upsert/delete by primary key, idempotent. (NOTE: LWW on `updated_at` for the control plane is STILL TO DO — today it is a last-applied-wins verbatim upsert; the true LWW arrives with the control-plane reconcile, decision 12.)
...
5. **Control-plane reconcile** 🔧 IN PROGRESS (decision 12): generic tombstones + full-snapshot LWW. Sub-chunks: (a) ✅ DONE `updated_at` as a DB COLUMN on credentials/users/teams (sqlite v20 / pg 0007, backfill = created_at) maintained on write (put_* = created_at, update_* and apply_remote_* = now()) — NOT in the Credential/User/Team structs (~78 literals avoided); it will travel in the snapshot's wire type, not in the entity; (b) ✅ DONE generic `control_tombstones(entity_type, entity_key, deleted_at)` table (sqlite v21 / pg 0008) + `ControlTombstoneStore` trait (record/list/delete/purge, idempotent) on sqlite+pg + tombstone recording on the deletes of the **5 primary entities** (credential/user/team/grant/bucket) in the decorators (cluster_control.rs + bucket in cluster_meta.rs), with symmetric tombstone cleanup on local re-creation (put_*/create_bucket). **SCOPE DECISION:** tombstones for join tables (team_members/user_grants/team_grants), bucket_config/bucket_tags and server_config are DEFERRED to sub-chunk (d), where the reconcile model that consumes them is decided (memberships = a set owned by the parent) — this avoids throwaway code. (c) ⏳ `GET /cluster/v1/control-snapshot` (entities + `updated_at` + tombstones) + `ClusterClient::fetch_control_snapshot` + `apply_*_at(entity, updated_at)`; (d) ⏳ a reconciliation pass in the anti-entropy worker (LWW merge on `updated_at` + apply tombstones + adopt peers' tombstones + GC, via `purge_control_tombstones`). Membership/join tables (team_members/user_grants/team_grants/bucket_tags): simpler to treat them as a set owned by the parent entity, replaced when the parent wins the LWW (avoids per-join timestamps); decide here also the tombstones for bucket_config/bucket_tags/server_config.

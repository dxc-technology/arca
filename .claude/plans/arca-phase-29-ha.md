...
Starting point: Arca already has 90% of the replication infrastructure (Phase 28). We reuse `OutboundClient` (SigV4) as the transport base, `ReplicationStore`+`JournalEntry`+`claim_batch`/backoff for recovery (hinted-handoff), the `x-amz-arca-replication-source` header for loop prevention, the credentials in `server_config`, and `/admin/health` (200 ok / 503 draining) as the health check for the LB.
...
- **Metadata = the hard problem**, confined to the `objects` table. Row `(bucket,key,version_id)` immutable; `is_latest` is a derived view = `MAX(last_modified, version_id)` recomputed locally. Verbatim replication with idempotent upsert; LWW conflicts on `(last_modified, version_id)` with a `write_id` tiebreak for null-version rows. Convergence without consensus.
...
Both modes share: discovery, verbatim replication, hinted-handoff, anti-entropy, LWW resolution. They differ ONLY in the write admission gate, in `ClusterMetadataStore::put_object` (the linearization point):
...
- **`mode="available"` (AP)**: the write is acknowledged after the local copy; fan-out to the other live nodes in parallel (best-effort, completed via hinted-handoff/anti-entropy). Always writable. In a partition both sides write; at heal LWW wins (a conflicting write on the same object is silently lost).
...
- **Per-node observability (NOT replicated)** — each node has its own, possibly aggregated externally:
  ...
  - **`replication_journal`** (Phase 28 and cluster hints): it is each node's LOCAL delivery state towards peers; replicating it would be wrong. Each node has its own.

The control plane changes rarely and is small: mutations propagate in real time like objects, but anti-entropy reconciles it with a **full set comparison** (few rows) instead of the objects' incremental changed-since. Control-plane mutations follow the same `mode` policy (in `quorum` they require the majority; in `available` always).
...
- `recompute_is_latest`: a shared pure function (one single place), used by single-node `put_object`, `apply_remote_object`, and the promotion in `delete_object_version`. Tiebreak `(last_modified DESC, version_id DESC)`.
- **Control plane — idempotent apply**: an `apply_control_op(entity, op, payload)` helper (or per-store `apply_remote_*` methods) on `CredentialStore`/`UserStore`/`TeamStore`/`GrantStore`/`ServerConfigStore` + bucket (`buckets`/`bucket_config`/`bucket_tags`), applying verbatim upsert/delete by primary key, idempotent, with LWW on `updated_at`.
- Migration (sqlite + pg): (a) on `objects` a `write_id TEXT` column (disambiguates LWW on null versions) and indexed `updated_at`/`seq` (changed-since scans), `write_id` in `ObjectRecord` (serde default, backward compatible); (b) add `updated_at` to the control-plane tables that lack it (`credentials`, `users`, `teams`, `team_members`) for uniform LWW + anti-entropy (defaulting to `created_at`/now, backward compatible).
...
- **Synchronized clocks (NTP)**: LWW compares wall-clock; severe skew can pick the wrong winner on null-version rows. Mitigated by the `write_id` tiebreak; HLC is future hardening.

...
### M4 — Resilience and catch-up ✅ COMPLETE (revised per decisions 8–12)
...
6. **Blob repair + GC** ✅ DONE: in the anti-entropy worker, on a slower cadence (every 10 ticks; O(n) scans). **Repair** (commit `ef5e9d5`, safe = it only adds bytes): `MetadataStore::list_referenced_blob_ids` (live non-tombstone object rows + in-flight parts); every blob physically missing locally is fetched from a peer (composites → fetch the missing PARTS). **GC** (dedicated commit, composite-aware + grace): pure function `plan_blob_gc(on_disk, referenced, now, grace)` in arca-core (tested) + `gc_blobs` in the worker. Anti-data-loss guard: the referenced set = blobs of metadata rows ∪ parts of composite sidecars whose composite blob is STILL referenced (parts of an orphaned composite stay recoverable); fail-safe: any enumeration error skips the whole round (never delete on a partial set); grace = `tombstone_grace` (an orphan blob must outlive the max reconcile lag). New `RawBlobOps`: `list_blob_ids` (file+mtime), `list_sidecar_ids`, `delete_blob_file` (NON-cascading). NOTE: offline `arca fsck`/`recover` remain non-composite-aware (TD-014 unchanged); only the cluster GC is.

*Outcome: a node that comes back realigns on its own — objects, control plane and blobs (durability + GC). M4 complete; M5 remains.*

### M5 — Console (topology), deploy, docs, tests 🔧 IN PROGRESS
- **Topology dashboard** ✅ DONE: the Server card's "Topology" field is now dynamic (`Cluster · live/total`, red when quorum is lost, otherwise "Single node"); **dedicated "Cluster Topology" bento box** (2-col span, only when `cluster.enabled`) with Mode (Quorum·W=N indigo / Available amber, shield/bolt icons), Write status (Writable green / Read-only red, check/exclamation icons), and the node list with green/red dot + "this node" badge + endpoint + relative last_seen. Replicates the existing glass/vault patterns; verified via Playwright in available (green) and quorum-no-majority (red). `console/js/views/dashboard.js`: loads `/admin/cluster` into `cluster` + a `relativeTime` helper.
- `GET /admin/cluster` ✅ DONE (admin-auth): `{enabled, mode, node_id, write_quorum, has_write_quorum, live_node_count, node_count, nodes:[{node_id,endpoint,alive,last_seen,local}]}`; `{enabled:false}` on single-node. Feeds the dashboard. (A dedicated `#/cluster` view + `arca cluster status` are still optional/TO DO.)
- `config/fragments/cluster.toml` ✅ DONE (single-node dev: available + mdns) + `bin/lib/compose.sh` `enable_cluster()` ✅ + `bin/arca --cluster` ✅ (mutually exclusive with `--replication`).
- **3-node cluster** ✅ DONE: `docker/docker-compose.cluster.yml` STANDALONE (not an overlay of the base file, avoids the conflict with the `arca` service): 3 services `arca-1/2/3` (YAML anchor, separate volumes, same symmetric config `docker/cluster/config.toml` = quorum/cluster_size=3/discovery="static"/seeds=the 3 service names, same `secret`, same root creds) + `arca-lb` (HAProxy `docker/cluster/haproxy.cfg`, `http-check ... GET /admin/health`, round-robin, exposes :9000) + console pointed at the LB. `bin/cluster up|down|status|logs|node-stop|node-start` (dedicated compose project `arca-cluster`). Verified via Playwright: 3/3 healthy (Quorum·W=2, Writable, all green), then `node-stop 3` → 2/3 (amber, arca-3 red "Offline", still Writable at 2/3). NOTE: encryption is not enabled in this test config (plain) → the "same master key" prerequisite is moot here; for an encrypted deployment it must be added. mDNS remains for bare-metal LANs (single-node dev uses mdns; the 3-node setup uses static because the Docker bridge does not pass multicast).
- Production deploy in `deploy/` ✅ DONE: `deploy/kubernetes/arca-cluster.yaml` (StatefulSet with 3 replicas + per-pod PVC for a stable node_id + headless Service for DNS discovery `discovery="dns"` + client Service + Secret with the shared config.toml and root creds, `/admin/health` probes, podManagementPolicy Parallel); `deploy/haproxy/` (`haproxy.cfg` + `keepalived.conf` VIP + `README.md`: 2 HAProxy LBs with a floating VIP via keepalived, round-robin, `httpchk GET /admin/health`); `deploy/config/arca-cluster.toml` (bare-metal node config, static discovery, paired with the existing systemd unit). YAML/TOML validated.
...
- Tests: unit (`recompute_is_latest` determinism; LWW; quorum math) ✅. **3-node integration ✅ DONE** — `bin/test cluster` (new `bin/test` target) orchestrates a self-contained cluster in PHASES, stopping/restarting nodes FROM THE SHELL between phases (a kill/restart cannot be driven from inside a test container). A `test` service was added to `docker/docker-compose.cluster.yml` (`test` profile, env `ARCA_ENDPOINT=http://arca-lb:9000` + `ARCA_NODE1/2/3_ENDPOINT`); tests in `tests/integration/test_cluster.py` (11 boto3 tests, topology markers registered in `conftest.py`). The `run`s use `--no-deps` so the LB stays up but stopped nodes are NOT resurrected by the `depends_on` chain. **6 phases, all green**: A=all up (replication on all nodes, read-after-write, failover seed); B=node 3 down (writable at 2/3 in quorum, failover read, catch-up write); C=nodes 2&3 down (write→503 ServiceUnavailable without quorum, read still OK); D=all up (catch-up converges on arca-3 via anti-entropy); E=overlay `docker-compose.cluster.507.yml` (arca-3 on a 16 MiB tmpfs → PUT > the smallest node's space = 507 InsufficientStorage end-to-end); F=overlay `docker-compose.cluster.drift.yml` (arca-3 with a different secret → `/admin/cluster` reports `config_aligned:false`). Test Coverage table in `README.md` updated ("Integration — HA Cluster" row, 11). The `available` 1/3-still-writable variant = NOT automated (it would require an available-mode test cluster; optional).

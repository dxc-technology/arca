...
- Migration (REVISED, decisions 10–11; in fact delivered in M4, not M2): (a) on `objects` ONLY the indexed `seq` column (changed-since cursor; sqlite v18 with a dedicated `object_seq` counter, pg 0005 with a `SEQUENCE`) — NO `write_id`, NO `updated_at` on objects (the LWW tiebreak is `blob_id`); `is_tombstone` added in sqlite v19 / pg 0006. (b) `updated_at` on the control-plane tables (`credentials`/`users`/`teams`) for the reconcile's LWW → STILL TO DO (sqlite v20 / pg 0007), in the control-plane reconcile sub-chunk.
...
5. **Control-plane reconcile** ⏳ TO DO (decision 12): generic tombstones + full-snapshot LWW. Sub-chunks: (a) `updated_at` as a DB COLUMN on credentials/users/teams (sqlite v20 / pg 0007) maintained on write — NOT in the Credential/User/Team structs (it would avoid ~78 literals to fix up); it travels in the snapshot's wire type, not in the entity; (b) a `control_tombstones(entity_type, entity_key, deleted_at)` table + CRUD store + recording on deletes in the decorators (cluster_control.rs + bucket in cluster_meta.rs); (c) `GET /cluster/v1/control-snapshot` (entities + `updated_at` + tombstones) + `ClusterClient::fetch_control_snapshot` + `apply_*_at(entity, updated_at)`; (d) a reconciliation pass in the anti-entropy worker (LWW merge on `updated_at` + apply tombstones + adopt peers' tombstones + GC). Membership/join tables (team_members/user_grants/team_grants/bucket_tags): simpler to treat them as a set owned by the parent entity, replaced when the parent wins the LWW (avoids per-join timestamps).

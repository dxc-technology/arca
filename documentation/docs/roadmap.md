...
- **`rustls-webpki` pinned to a vulnerable version transitively** (TD-024): `rumqttc` 0.25.1 (MQTT connector) hard-pins `rustls-webpki = "^0.102.8"`, which carries 4 published advisories; `rumqttc` 0.25.1 is the latest release and never allows `>=0.103`. No `[patch]` override applied without review. Track upstream `rumqttc` releases or evaluate its `native-tls` backend.

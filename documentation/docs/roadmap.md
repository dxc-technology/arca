...
- [ ] R8 — Console: node selector for the node-local views behind the load balancer (audit, metrics history, notification events, replication journal) via a server-side `?node=` proxy, including an "All nodes" merged view (decision H9)

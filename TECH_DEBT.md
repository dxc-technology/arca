...
| TD-015 | Cluster inter-node TLS cert verification | The cluster membership health-probe and the cluster transport client build their `reqwest` clients with `danger_accept_invalid_certs(true)` so peers presenting self-signed certs under clustered TLS can be reached. Inter-node requests are authenticated by SigV4 + the shared `[cluster].secret` (signed headers), not by TLS, so accepting the cert does **not** weaken request authentication. **Severity revised 2026-06-11 (review §3.7): the risk is NOT confidentiality-only.** TLS does not authenticate the peer, and the fan-out push direction authenticates no peer at all (`live_peers()` filters on `alive` only; membership admits an mDNS peer on a `cluster_id` match), so a rogue peer can receive all new data with no secret. Verified mutual TLS with a shared cluster CA is the cleanest fix for both: a node would verify a peer's CA-signed cert before fanning out, closing §3.7(A) and the passive-sniff / active-MITM exposure §3.7(C). Now a **committed** item, not deferred. | Distribute a shared cluster CA via config, build the cluster `reqwest` clients with that root, verify the peer cert before adding it to membership / fanning out, and drop `danger_accept_invalid_certs`. Scheduled in the HA hardening plan R4 (`.claude/plans/arca-phase-29-ha-hardening.md`). | None | `crates/arca-server/src/cluster/membership.rs`, `crates/arca-server/src/cluster/client.rs` |
| TD-016 | Cluster control-plane reconcile is partial | The anti-entropy worker reconciles (full-snapshot LWW + tombstones) ONLY the 5 tombstoned control-plane families: credentials, users, teams, grants, buckets. Memberships / grant-attachments (`team_members`, `user_grants`, `team_grants`), `bucket_config`, `bucket_tags`, and `server_config` replicate in REAL TIME (best-effort fan-out) but are NOT part of the snapshot reconcile. So a node that was down/unreachable when one of those changed can stay diverged on it until the next write touches that entity — real-time replication is the only path that heals them. At N=3 with brief outages this is low-risk, but it breaks the "a returning node fully self-heals" guarantee for those families (Phase 29 HA, M4). | Extend `build_control_snapshot` + `plan_control_merge` to cover these: memberships/grant-attach as a parent-owned set (replace the child set when the parent entity is the LWW winner, avoiding per-join timestamps); `bucket_config`/`bucket_tags` via their `updated_at`; `server_config` via per-key LWW (respecting the `node_id` denylist). Add tombstones where a delete must propagate (detach/remove-member/config-delete). | None (runtime + real-time replication unaffected; only post-downtime convergence for these families) | `crates/arca-server/src/cluster/anti_entropy.rs`, `crates/arca-storage/src/{sqlite,pg}/control_snapshot.rs`, `crates/arca-core/src/cluster.rs` (`plan_control_merge`) |

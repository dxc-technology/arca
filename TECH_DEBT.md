...
| TD-005 | Request ID mismatch | Error XML body contains a UUID generated at `S3Error::new()` time, while `x-amz-request-id` response header is set by middleware — two different UUIDs | Pass the middleware-generated request ID into the error response builder so both match | `test_object_requestid_matches_header_on_error` | `error.rs`, `request_id.rs`, `error_response.rs` |

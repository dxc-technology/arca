...
- **(Console)** Per-bucket **Replication** card under Bucket Settings (between Event Notifications and Danger Zone). Amber "Versioning required" banner with an inline "Enable versioning" link that smooth-scrolls to the versioning card. Rule rows show a `source ➜ destination` flow with a tiny accent arrow; each rule has an inline Enabled/Disabled toggle. Modal editor auto-generates the Rule ID, emerald-bordered Status toggle, Destination (Bucket + Region + Endpoint URL) with a tiny emerald "Loop prevention active" hint when the URL looks like an Arca endpoint, and a Credential Ref dropdown with an inline "+ New" flow that POSTs the credential and immediately selects it before saving the rule.

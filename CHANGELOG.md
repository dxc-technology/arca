...
- **`arca tls generate` wrote every file world-readable, private keys included.** All four outputs of `arca tls generate` — and the CA plus every per-node key of `arca tls generate-cluster` — went through `std::fs::write`, which creates a file `0666 & ~umask`, i.e. `0644` under the usual umask. The private keys were therefore readable by every local user on the host and by every process in any container sharing that directory. Modes are now explicit and split by role: certificates `0644` (public material), server and node private keys `0640`, the CA private key `0600`. The server key is deliberately group-readable rather than owner-only, because the group *is* the access channel: Arca runs as `65532:65532` and the web console runs as a different user (`100:101`), and in the Compose dev setup both read the same key — the console reaches it through a supplementary group, never through world-readable bits. The CA key is `0600`: nothing reads it at runtime, it only signs. The mode is applied at creation time (`OpenOptions::mode`) so a key is never even briefly world-readable, and re-applied afterwards for two reasons — the creation mode is masked by the process umask, which must not be allowed to strip the group bit a key needs, and it does not apply at all to a file left behind by an earlier run, which is what makes re-running `arca tls generate` tighten keys an older version wrote loose. **Action required:** keys generated by an earlier version keep their old mode until they are regenerated or `chmod`'d by hand; treat a key that has been sitting world-readable on a shared host as disclosed and rotate it.

### Fixed

- **The Kubernetes console deployment could not start.** `deploy/kubernetes/arca-console.yaml` set `runAsUser: 101`, but 101 is the nginx *group* — the image's user is uid **100**. Running as 101 fails at startup: the entrypoint cannot rewrite `index.html` (`sed: can't create temp file`) and nginx cannot create `/var/lib/nginx/tmp/client_body`, because the image pre-owns those paths as `100:101`. Corrected to `runAsUser: 100`, `runAsGroup: 101`, and the misleading comment on that line with it. The pod also gained `fsGroup: 65532` so a mounted certificate Secret is readable when the console terminates TLS itself rather than at the ingress; `arca-cluster.yaml`, which was already correct at `65532` throughout, is unchanged.

- **The console never enabled HTTPS with certificates produced by `arca tls generate`.** Its entrypoint skips CA material when auto-detecting the certificate and key, and did so by matching the bare substring `ca` against the file name — which matches `arca-server.crt` and `arca-server.key`, inside the word "arca". Every file the `tls-init` service writes was therefore skipped, no certificate was found, and nginx quietly kept serving plain HTTP while port 9443 refused connections. The match is now against a `ca` *word* in the base name (`ca.pem`, `arca-ca.crt`, `client-ca.pem`, `ca-bundle.crt`), so CA files are still excluded and server files are not. The bug was masked for anyone whose `certs/` directory also held real certificates under names like `fullchain.pem`/`privkey.pem`, which the old glob happened not to match. The entrypoint additionally no longer degrades silently when the key is present but unreadable: it logs the uid and groups it is running as and how to grant the missing one, instead of leaving "port 443 is closed" as the only symptom.

### Changed

- **The console reaches Arca's TLS private key through a shared group instead of world-readable bits.** Arca's images run as `65532:65532`, the console as `100:101`, and the Compose dev flow bind-mounts one `certs/` directory into both — no owner and mode can grant read to both users without also granting it to everyone. The key is now group-readable by `65532` (Arca's service GID), and the console is given that GID as a supplementary group: `group_add: ["65532"]` in `docker/docker-compose.tls.yml`, `fsGroup: 65532` in `deploy/kubernetes/arca-console.yaml` — the same mechanism Kubernetes provides natively. Note that 65532 is the distroless `nobody` GID and is documented as "the identity Arca runs under", not as a privilege boundary. The [TLS guide](https://github.com/dxc-technology/arca/blob/main/documentation/docs/guide/tls.md) gained a **File Permissions** section stating the rule once (*group-readable, never other-readable*), the expected mode of each file, the Compose and Kubernetes snippets, and — the part that actually bites in production — a certbot `--deploy-hook` example, because a renewal rewrites `privkey.pem` as `root:root 0600` and silently reverts a one-time `chgrp`, breaking TLS 60-90 days after the change that caused it. `setfacl -m g:65532:r` is documented as the alternative where group ownership cannot be changed.

### Added

- **`publiccode.yml`** — the metadata file that describes Arca to the [Developers Italia](https://developers.italia.it) open source software catalogue, written against version 0.7.0 of the [publiccode.yml standard](https://yml.publiccode.tools) and validated with the official parser (`italia/publiccode-parser-go`). It declares the software as a stable standalone backend owned by DXC Technology, under AGPL-3.0-or-later, and carries a user-facing description, the feature list, the documentation links and the console screenshots. Two of its keys, `softwareVersion` and `releaseDate`, go stale at every release and are now part of the version-location checklist in `RELEASING.md`; `releaseDate` in particular holds no version string, so the usual "grep for the old version" sweep does not catch it. Note that the catalogue's crawler only scans repositories registered against a publisher, so the file alone does not list Arca in the catalogue.

- **AI-generated code disclosure.** A new `NOTICE` file at the repository root states that substantially all of this repository - source code, tests, configuration, scripts and documentation - was generated with the assistance of AI coding tools, under human direction and review, and that the disclosure does not alter the AGPL-3.0-or-later terms. The notice is surfaced wherever a reader is likely to look rather than repeated in every one of the ~200 source files: an info callout in the License section of `README.md` and of the documentation home page, the MkDocs `copyright` footer so it renders on **every** documentation page, a `//!` line in each of the five crate roots (so it also appears in `cargo doc` output), and a comment in `console/index.html` and `console/js/app.js`.

- **Trivy filesystem-scan configuration** (`trivy.yaml`, `trivy-secret.yaml`, `.trivyignore`) — `trivy fs .` previously reported a fixed root/test credential (`AKIA5B6BSHJA8CIHZSVG` and its secret key) used throughout `tests/`, the Compose test services, and the console login placeholder, as 9 leaked-secret findings; it is a value Arca itself generates for local containers, never a real AWS credential, so `trivy-secret.yaml` now allow-lists it by exact value wherever it appears. The scan also reported the 4 `rustls-webpki` advisories already tracked and dismissed as unreachable in TD-024; `.trivyignore` suppresses those same 4 GHSA IDs, pointing back at that entry. Two remaining findings — the console Kubernetes manifest and a handful of test-only Dockerfiles — are real, minor gaps left visible and tracked as TD-027/TD-028 rather than suppressed.

- **`bin/test tls-permissions`** — a self-contained suite for the permission semantics above, because nothing else could catch a regression in them. The existing TLS tests run against bind mounts, and Docker Desktop on macOS fakes bind-mount ownership: a `0600` key owned by the host user appears owned by whatever UID the container runs as and reads fine, so any uid/gid assertion built on one would pass vacuously on a developer Mac and fail only on a real Linux host. The new suite stages its material in a **named Docker volume**, where ownership is real on every host, and runs nine checks: the four modes and the owning GID that `arca tls generate` writes, seen from inside a container rather than from the host; Arca (65532) serving HTTPS from the `0640` key; the console (uid 100) serving HTTPS *with* the service GID; and — so the suite has teeth — the console *without* it failing to enable TLS, plus the bare read of the key failing and succeeding in the same two configurations.

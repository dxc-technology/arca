...
- **Admin API**: `GET /admin/replication/journal` (filters: bucket, status, rule_id, pagination), `POST /admin/replication/credentials/{name}` + `DELETE /admin/replication/credentials/{name}` for destination-credential management (stored encrypted as `replication.credentials.<name>` in `server_config`), `POST /admin/replication/retry/{id}` to requeue a failed entry. `/admin/info` gained a `replication_enabled: bool`.
...
- **(Console)** New admin view at `#/replication` — **Replication Journal**. Inline column-header filters (Created, Bucket, Key, Rule, Event Type, Destination, Status), a color-coded status chip (pending / in_flight / completed / failed) with a subtle pulse dot on `in_flight`, side detail panel with a dedicated "Flow" summary card (bucket ➜ destination, status + event-type side by side), pagination and auto-refresh every 30 s. Failed rows have a per-row "Retry" button and a "Retry delivery now" CTA inside the side panel.

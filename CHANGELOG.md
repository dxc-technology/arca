...
- **All four images now scan clean (`trivy image`, 0 findings).** Two gaps remained after the rebuilds above. The console's base still shipped openssl 3.5.7-r0, leaving 20 findings (`libcrypto3`/`libssl3`, fixed in 3.5.8-r0) that the previous `nginx:alpine` base carried too — the earlier scan had simply not listed them, reporting only `libuuid`. And both Python images shipped `pip`, whose vendored `msgpack` and `setuptools` copies trail their own advisories (GHSA-6v7p-g79w-8964, CVE-2025-47273, CVE-2026-59890); nothing needs pip at runtime, so it is removed once the requirements are installed. Remediation is by **pinned point upgrade**, never a blanket `apk upgrade`: each Dockerfile names the individual package and the advisory that forced it (`apk add 'libcrypto3>=3.5.8-r0' 'libssl3>=3.5.8-r0'`, `apk add 'libuuid>=2.42.3-r1'`), so a build pulls a known set of versions and the diff records why. `>=` rather than `=` because Alpine drops superseded versions from its repo index, which would turn an exact pin into a build failure the day a newer build lands. Where no fix exists yet the finding is accepted and tracked in `TECH_DEBT.md` rather than papered over with a blanket upgrade. For reference, `trivy` on the pre-change console reported 52 findings across `curl`, `libcurl`, `libcrypto3`, `libssl3`, `libuuid` and `libexpat`. The console image grows 12.6 → 18.4 MB (replacing base packages rewrites them in a new layer); still well under a third of the original 65.5 MB.

- **Connector-test receiver images rebuilt on the same hygiene rules.** `webhook-receiver` and `grpc-receiver` carried the same `libuuid` advisories. Unlike the console, the dependency is genuine there — CPython's `_uuid` extension links `libuuid.so.1` — so the package is upgraded (a pinned `apk add 'libuuid>=2.42.3-r1'`, from the base image's 2.42.1-r0) rather than removed, and both images move to the current `python:alpine`. `grpc-receiver` additionally dropped the `gcc`/`g++`/`musl-dev`/`libffi-dev`/`linux-headers` toolchain it kept in the runtime layer: musl wheels now exist for `grpcio`, so `pip install --only-binary=:all:` never compiles from source, and stub generation moved to a builder stage that keeps `grpcio-tools` out of the runtime — 820 MB → 90 MB. `grpcio` 1.66.2 → 1.84.0 and `protobuf` 5.29.6 → 7.36.1 along the way. Neither image is ever distributed; both exist only for `bin/test notifications` and `bin/test connectors grpc`, which pass unchanged (20 and 7 tests). The `webhook-receiver` compose service also gained an explicit `image: arca-webhook-receiver`, matching every other locally built service — without it compose derived the name from the project directory and an image built under the old name lingered as an untracked orphan, which is how a stale copy stayed on disk long enough to be scanned.
...
- **`DeleteObjects` batch conditional deletes on a specific version checked the bucket's *latest* object instead of the *targeted* version.** A version-specific entry with an `ETag`/`LastModifiedTime`/`Size` precondition was validated against `get_latest_object`, so deleting an older version whose ETag matched could be wrongly refused (or, on a versioned bucket with tags, another version's stale metadata could wrongly pass). It now checks the exact version being deleted, matching the plain (non-batch) `DeleteObject` path.
- **PostgreSQL backend: conditional writes were not actually serialized under concurrency.** The new authoritative CAS check (above) ran its precondition read *before* `next_object_seq()` — the single statement in each transaction that actually contends under PostgreSQL's default READ COMMITTED isolation, since a plain `SELECT` does not block concurrent transactions. Concurrent writers for the same key could all read the same pre-write snapshot, all pass their precondition, and commit one after another, reproducing the exact check-then-commit race the fix above exists to close — on PostgreSQL only (SQLite's single-connection actor already serialized every statement regardless of order). Fixed by taking the seq first in `put_object_if`, `delete_object_if` and `delete_object_version_if`, turning it into a serialization gate so each transaction's precondition check is guaranteed to see genuinely current state. This also closes a gap in `delete_object_if`'s non-cluster unversioned-delete path, which previously took no serializing statement at all.
- **Known limitation, tracked as TD-025:** in a cluster, conditional-write atomicity is exact **per node**, not cluster-wide — `put_object`/`delete_object` commit locally then fan out, with no per-key authoritative node to arbitrate across peers. Two conditional writes racing the same base ETag on two *different* nodes can both be accepted, converging afterward by last-writer-wins. Mitigated by pinning a key's conditional-write traffic to one node via sticky load-balancer routing; see the [HA guide](https://github.com/dxc-technology/arca/blob/main/documentation/docs/guide/ha.md#design-trade-offs-and-honest-limitations).
...
- **Dependency vulnerability remediation.** `quick-xml` 0.40.1 → 0.41.0 (RUSTSEC-2026-0194, RUSTSEC-2026-0195), `h2` 0.4.14 → 0.4.17 (RUSTSEC-2026-0258, transitive via `hyper`), `crossbeam-epoch` 0.9.18 → 0.9.20 (RUSTSEC-2026-0204, transitive via `moka`), `event-listener` 5.4.1 → 5.4.2 (RUSTSEC-2026-0221, transitive via `async-lock`/`lapin`/`moka`/`sqlx-core`). The `grpc-receiver` connector-test tooling's `protobuf` bumped 5.28.2 → 5.29.6 (CVE-2025-4565, CVE-2026-0994) — library-only, never part of the distributed `arca` binary. `rustls-webpki` 0.102.8 (4 published advisories, transitive via `rumqttc`'s MQTT connector) remains unfixed: `rumqttc` 0.25.1, the latest published release, hard-pins `rustls-webpki = "^0.102.8"` with no newer version available — tracked as TD-024 rather than patched with an unvetted override.
- **All Docker images now run as non-root by default.** The production (`scratch`) and development (`debian:stable-slim`) Arca runtimes, the nginx-based console, and every auxiliary test/tooling image (`Dockerfile.test`, `Dockerfile.perf`, `grpc-receiver`, `webhook-receiver`, `s3-tests`) drop root and run as a fixed non-root user — except `s3-tests`, which runs as the invoking host user (`docker/docker-compose.yml`'s `S3TESTS_UID`/`S3TESTS_GID`, set by `bin/s3-tests`) since it writes results into a bind-mounted host directory. `HEALTHCHECK` instructions were added to every long-running server image that lacked one. See the [TLS guide](https://github.com/dxc-technology/arca/blob/main/documentation/docs/guide/tls.md) for a permissions caveat this introduces for bind-mounted certificate files on Linux hosts.
- **Kubernetes manifests hardened.** `deploy/kubernetes/arca-cluster.yaml` and `arca-console.yaml` gained pod- and container-level `securityContext`: `runAsNonRoot`, explicit `runAsUser`/`runAsGroup` matching each image's built-in user, dropped Linux capabilities, `seccompProfile: RuntimeDefault`, and (for the `arca` container, whose only writable path is the `/data` volume) `readOnlyRootFilesystem: true`.
- **`THIRD-PARTY-NOTICES.md` regenerated.** It had silently gone stale since the `license` field was added in a prior commit: `cargo-about`'s rule for skipping Arca's own crates keys on `publish = false`, which that commit never set, so every generation attempt since has been failing at the root-cause level rather than just being out of date. Fixed by adding `publish = false` to `[workspace.package]` and `publish.workspace = true` to all five crates.
- **Bootstrap credential no longer uses AWS's published SigV4 documentation example.** Every dev/test default (Docker Compose, `s3-tests` config, console login-form placeholder, screenshot automation, storage unit-test fixtures, integration/perf test defaults, and the admin-API docs example) used `AKIAIOSFODNN7EXAMPLE` / `wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY`, which secret scanners and password managers flag as a known/compromised credential even though it was never a real secret. Replaced with a freshly generated random credential everywhere it served as a bootstrap default. The same string still appears, deliberately unchanged, in `crates/arca-auth`'s SigV4 conformance tests, where it is AWS's own published test vector with hardcoded expected signatures rather than a credential.

...
- **Change journal** — new `replication_journal` table (SQLite migration v17 / Postgres `0004_replication.sql`) records every pending/in-flight/failed delivery. Rows transition `pending → in_flight → completed | failed`; Postgres uses `FOR UPDATE SKIP LOCKED` when claiming a batch. Exponential backoff per entry, capped at one hour; `FAILED` only stamped after the terminal attempt so clients don't see transient failures.
- **Replication worker** — DB-polled, hourly-capped backoff, per-event-type dispatch (PUT / delete-marker / tag). Conflict resolution is last-writer-wins by `Last-Modified`: the worker HEADs the destination before a PUT and skips when destination is newer. Audit entries are written on every success/failure.
- **Loop prevention** — every outbound request carries `x-amz-arca-replication-source: <source_endpoint_id>`. Receiving Arca flags the object `REPLICA` and skips journal emission. Mirror setups converge end-to-end: integration-tested with both instances pointing at each other with the same bucket name — a single PUT on the source reaches the mirror exactly once and never bounces back.
- **`x-amz-replication-status` response header** on `PutObject`, `GetObject`, `HeadObject`, `CopyObject`, `CompleteMultipartUpload`. Values: `PENDING` / `COMPLETED` / `FAILED` / `REPLICA`. New `replication_status` column on `objects`; `ObjectRecord` gained the field.
- **Admin API**: `GET /admin/replication/journal` (filters: bucket, status, rule_id, pagination), `GET /admin/replication/credentials` (list — name + access_key_id only, secret never returned), `GET /admin/replication/credentials/{name}/usage` (list of rules referencing the credential, per bucket) and `POST /admin/replication/credentials/{name}` + `DELETE /admin/replication/credentials/{name}` for destination-credential management (stored as `replication.credentials.<name>` in `server_config`). Deleting a credential now disables every replication rule that references it before removing the record, so the worker can't loop forever retrying against a vanished reference — the response body lists what was touched. `POST /admin/replication/retry/{id}` to requeue a failed entry. `/admin/info` gained a `replication_enabled: bool`.
- **Outbound SigV4 signer** — new `sign_outbound_request` in `arca-auth` reusing the private canonical-request helpers; round-tripped against `verify_request` in unit tests.
- **`PutBucketReplication` / `GetBucketReplication` / `DeleteBucketReplication`** S3 handlers with Arca-flavoured XML extensions (`<Endpoint>`, `<CredentialRef>`, `<Region>` alongside the standard `<Bucket>`). New `ReplicationConfigurationNotFoundError` S3 error code.
- **`[replication]` TOML section**: `poll_interval_seconds` (default 15), `batch_size` (100), `max_retries` (10), `retry_base_seconds` (5), `request_timeout_seconds` (60), `source_endpoint_id` (loop-prevention identifier), `journal_retention_days` (30) and `journal_max_age_days` (90). The last two feed the existing retention worker so `completed` rows are pruned after 30 days and the table is hard-capped at 90 regardless of status, keeping the journal bounded even during an extended destination outage.
- **Docker replication test mode** — `docker-compose.replication.yml` boots a second `arca-replica` instance on `:9001`; `bin/arca start --replication` starts both; `bin/test replication` runs the full integration suite (two-instance boto3 harness). 4 new boto3 integration tests covering basic PUT replication, delete-marker propagation, tag replication, and the two-way mirror no-loop invariant.
- **(Console)** Per-bucket **Replication** card under Bucket Settings (between Event Notifications and Danger Zone). Amber "Versioning required" banner with an inline "Enable versioning" link that smooth-scrolls to the versioning card. Rule rows show a `source ➜ destination` flow with a tiny accent arrow; each rule has an inline Enabled/Disabled toggle and chips for any tag-filter entries. Modal editor auto-generates the Rule ID, emerald-bordered Status toggle, **Prefix + Tag filter** editor (add/remove tag rows, duplicate-key validation, serializes as `<Prefix>` / `<Tag>` / `<And>` as appropriate — matching AWS + MinIO semantics), Destination (Bucket + Region + Endpoint URL) with a tiny emerald "Loop prevention active" hint when the URL looks like an Arca endpoint, and a Credential Ref dropdown with an inline "+ New" flow that POSTs the credential and immediately selects it before saving the rule.
- **(Console)** New admin view at `#/replication` — **Destination Credentials** card above the **Replication Journal**. The credentials card lists every stored `replication.credentials.*` entry (name + access-key-id; the secret is never exposed), with a `+ New credential` modal and per-row delete confirmation. The rule-editor modal inside bucket settings pulls this global list on open, replacing the previous "infer from existing rules" behaviour. Journal: inline column-header filters (Created, Bucket, Key, Rule, Event Type, Destination, Status), a color-coded status chip (pending / in_flight / completed / failed) with a subtle pulse dot on `in_flight`, side detail panel with a dedicated "Flow" summary card (bucket ➜ destination, status + event-type side by side), pagination and auto-refresh every 30 s. Failed rows have a per-row "Retry" button and a "Retry delivery now" CTA inside the side panel.
- **(Docs)** New `guide/replication.md` covering setup, the loop-prevention contract, two-way mirror configuration, destination-credential management, the journal and retry workflow. Updated `guide/configuration.md` and the CLI reference with the `--replication` flag and `[replication]` section.

### Changed

- `ObjectRecord` gained a `replication_status: Option<String>` field. Wire format and existing migrations are unchanged; all existing constructors populate it with `None`.
- `OBJECT_COLUMNS` constants in `arca-storage/sqlite/metadata.rs` and `arca-storage/pg/metadata.rs` extended with `replication_status`; `row_to_object_record` reads it.
- Extended the `StoreSet` / `build_store_set` trait-object bundle to include `arca_core::store::ReplicationStore`, wired as `state.replication_store` on `AppState`.
